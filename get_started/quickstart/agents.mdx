...
**dynamic_params**: a dictionary that contains key value pairs (paramter name : description) of the API endpoint's mutable parameters that need to be set by REBEL in order to answer a query.

**method**: GET/POST, whichever is the type of the API endpoint.

**url**: URL of the API endpoint that the given tool specifies.

**static_params**: any params that are constant between all API calls. An API key/token is an example of this.
