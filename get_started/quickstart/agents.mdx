...
**dynamic params**: a dictionary that contains key value pairs {parameter:description} of mutable parameters to the API endpoint that need to be set by REBEL to answer a query.
**method**: GET/POST, whichever is the type of the API endpoint.
**url**: URL of the API endpoint that the given tool specifies.
**static params**: any params that are constant between all API calls. An API key/token is an example of this.
